...
create table if not exists public.school_requests (
  ...
  fee_model text not null default 'keep_100',
...
alter table public.fundraisers
  add column if not exists fee_model text not null default 'keep_100';
alter table public.fundraisers
...
-- Campaign window: no logging texts or donations outside fundraiser dates (Central Time).
